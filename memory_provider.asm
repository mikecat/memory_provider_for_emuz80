...
GPR_LAST = GPR_LAST + SIZE
	...
	ALLOCATE_GPR FREQUENCY_DIVIDEND, 4
	ALLOCATE_GPR FREQUENCY_VALUE, 3
	ALLOCATE_GPR FREQUENCY_DELTA, 3
	ALLOCATE_GPR FREQUENCY_POSTSCALE, 3
	ALLOCATE_GPR FREQUENCY_POSTSCALE_COUNT, 1
	ALLOCATE_GPR FREQNEUCY_NONZERO_FOUND, 1
	...
	ALLOCATE_GPR CONFIG_SAVE_COUNTER, 1
	ALLOCATE_GPR CONFIG_SAVE_OK, 1
	ALLOCATE_GPR EDIT_FREQUENCY_BUFFER, 8 * 2 - 1
	ALLOCATE_GPR EDIT_FREQUENCY_COUNT, 1
	ALLOCATE_GPR EDIT_FREQUENCY_INPUT_WORK, 1
...
EEPROM_FOR_PORT_OFF_OK
	...
	; skip NCO configuration when clock output is disabled
	MOVF GPR_CLOCK_ENABLE, W, A
	BTFSC STATUS, Z, A
	BRA NCO_CONFIGURE_END
	; skip if the frequency set is zero
	MOVF GPR_CLOCK_FREQUENCY, W, A
	IORWF GPR_CLOCK_FREQUENCY + 1, W, A
	IORWF GPR_CLOCK_FREQUENCY + 2, W, A
	IORWF GPR_CLOCK_FREQUENCY + 3, W, A
	BTFSC STATUS, Z, A
	BRA NCO_CONFIGURE_END
	; convert BCD frequency to 32-bit integer
	CLRF FREQUENCY_DIVIDEND, A
	CLRF FREQUENCY_DIVIDEND + 1, A
	CLRF FREQUENCY_DIVIDEND + 2, A
	CLRF FREQUENCY_DIVIDEND + 3, A
	MOVLW 4
	MOVWF FREQUENCY_POSTSCALE_COUNT, A
	SET_FSR 0, GPR_CLOCK_FREQUENCY
FREQUENCY_INIT_LOOP
	; frequency *= 100
	MOVLW D'100'
	MULWF FREQUENCY_DIVIDEND, A
	MOVFF PRODL, FREQUENCY_DIVIDEND
	MOVFF PRODH, FREQUENCY_DELTA
	MULWF FREQUENCY_DIVIDEND + 1, A
	MOVF PRODL, W, A
	ADDWF FREQUENCY_DELTA, W, A
	MOVWF FREQUENCY_DIVIDEND + 1, A
	MOVFF PRODH, FREQUENCY_DELTA
	MOVLW D'100'
	MULWF FREQUENCY_DIVIDEND + 2, A
	MOVF PRODL, W, A
	ADDWFC FREQUENCY_DELTA, W, A
	MOVWF FREQUENCY_DIVIDEND + 2, A
	MOVFF PRODH, FREQUENCY_DELTA
	MOVLW D'100'
	MULWF FREQUENCY_DIVIDEND + 3, A
	MOVF PRODL, W, A
	ADDWFC FREQUENCY_DELTA, W, A
	MOVWF FREQUENCY_DIVIDEND + 3, A
	; read 2 digits of frequency
	MOVFF POSTINC0, FREQUENCY_DELTA
	; multiply the higher digit by 10
	SWAPF FREQUENCY_DELTA, W, A
	ANDLW 0xF
	MOVWF FREQUENCY_DELTA + 1, A
	RLNCF FREQUENCY_DELTA + 1, F, A
	RLNCF FREQUENCY_DELTA + 1, F, A
	ADDWF FREQUENCY_DELTA + 1, F, A
	RLNCF FREQUENCY_DELTA + 1, F, A
	; add the lower digit
	MOVF FREQUENCY_DELTA, W, A
	ANDLW 0xF
	ADDWF FREQUENCY_DELTA + 1, W, A
	; add the result to the frequency
	ADDWF FREQUENCY_DIVIDEND, F, A
	MOVLW 0
	ADDWFC FREQUENCY_DIVIDEND + 1, F, A
	ADDWFC FREQUENCY_DIVIDEND + 2, F, A
	ADDWFC FREQUENCY_DIVIDEND + 3, F, A
	; proceed to next 2 digits of frequency
	DECF FREQUENCY_POSTSCALE_COUNT, F, A
	BNZ FREQUENCY_INIT_LOOP
	; initialize division
	CLRF FREQUENCY_VALUE, A
	CLRF FREQUENCY_VALUE + 1, A
	CLRF FREQUENCY_VALUE + 2, A
	CLRF FREQUENCY_DELTA, A
	CLRF FREQUENCY_DELTA + 1, A
	MOVLW 0x08
	MOVWF FREQUENCY_DELTA + 2, A
	MOVLW 1
	MOVWF FREQUENCY_POSTSCALE, A
	CLRF FREQUENCY_POSTSCALE + 1, A
	CLRF FREQUENCY_POSTSCALE + 2, A
	CLRF FREQUENCY_POSTSCALE_COUNT, A
	CLRF FREQNEUCY_NONZERO_FOUND, A
	; perform division: value = dividend / 16,000,000 (0x00f42400)
FREQUENCY_DIVISION_LOOP
	; check if dividend >= 16,000,000
	TSTFSZ FREQUENCY_DIVIDEND + 3, A
	BRA FREQUENCY_DIVISION_GE ; jump if dividend[3] != 0x00
	MOVLW 0xF4
	SUBWF FREQUENCY_DIVIDEND + 2, W, A ; dividend[2] - 0xF4
	BNC FREQUENCY_DIVISION_L ; jump if dividend[2] < 0xF4
	CPFSEQ FREQUENCY_DIVIDEND + 2, A
	BRA FREQUENCY_DIVISION_GE ; jump if dividend[2] > 0xF4 (don't jump if dividend[2] != 0xF4)
	MOVLW 0x24
	SUBWF FREQUENCY_DIVIDEND + 1, W, A ; dividend[1] - 0x24
	BNC FREQUENCY_DIVISION_L ; jump if dividend[1] < 0x24
	; here, dividend[3:2] == 0x00f4 && dividend[1] >= 0x24
	; therefore, we can say that dividend >= 0x00f42400
FREQUENCY_DIVISION_GE
	; if this is first "1" bit, and postscaler is used, perform some adjustment
	; because it don't work well if first NCO overflows every clocks
	MOVF FREQNEUCY_NONZERO_FOUND, W, A
	BNZ FREQUENCY_DIVISION_GE_NO_ADJUSTMENT
	MOVF FREQUENCY_POSTSCALE_COUNT, W, A
	BZ FREQUENCY_DIVISION_GE_NO_ADJUSTMENT
	; shift the delta
	RRNCF FREQUENCY_DELTA + 2, F, A
	; decrement the postscaler
	BCF STATUS, C, A
	RLCF FREQUENCY_POSTSCALE, F, A
	RLCF FREQUENCY_POSTSCALE + 1, F, A
	RLCF FREQUENCY_POSTSCALE + 2, F, A
	BTFSS FREQUENCY_POSTSCALE + 2, 4, A
	BRA FREQUENCY_DIVISION_GE_NO_ADJUSTMENT
	CLRF FREQUENCY_POSTSCALE + 2, A
	BSF FREQUENCY_POSTSCALE, 0, A
	DECF FREQUENCY_POSTSCALE_COUNT, F, A
FREQUENCY_DIVISION_GE_NO_ADJUSTMENT
	; append bit "1" to the result
	MOVF FREQUENCY_DELTA, W, A
	IORWF FREQUENCY_VALUE, F, A
	MOVF FREQUENCY_DELTA + 1, W, A
	IORWF FREQUENCY_VALUE + 1, F, A
	MOVF FREQUENCY_DELTA + 2, W, A
	IORWF FREQUENCY_VALUE + 2, F, A
	; subtract 16,000,000 from the dividend
	MOVLW 0x00
	SUBWF FREQUENCY_DIVIDEND, F, A
	MOVLW 0x24
	SUBWFB FREQUENCY_DIVIDEND + 1, F, A
	MOVLW 0xF4
	SUBWFB FREQUENCY_DIVIDEND + 2, F, A
	MOVLW 0x00
	SUBWFB FREQUENCY_DIVIDEND + 3, F, A
	; set nonzero found flag
	SETF FREQNEUCY_NONZERO_FOUND, A
	BRA FREQUENCY_DIVISION_COMMON
FREQUENCY_DIVISION_L
	; if nonzero has been found, just skip (append bit "0" to the result)
	TSTFSZ FREQNEUCY_NONZERO_FOUND, A
	BRA FREQUENCY_DIVISION_COMMON
	; increment the postscaler
	BCF STATUS, C, A
	RRCF FREQUENCY_POSTSCALE + 2, F, A
	RRCF FREQUENCY_POSTSCALE + 1, F, A
	RRCF FREQUENCY_POSTSCALE, F, A
	; if the carry flag is set, increment the postscaler counter and set MSB
	BNC FREQUENCY_DIVISION_COMMON
	INCF FREQUENCY_POSTSCALE_COUNT, F, A
	BSF FREQUENCY_POSTSCALE + 2, 3, A
FREQUENCY_DIVISION_COMMON
	; shift the dividend
	BCF STATUS, C, A
	RLCF FREQUENCY_DIVIDEND, F, A
	RLCF FREQUENCY_DIVIDEND + 1, F, A
	RLCF FREQUENCY_DIVIDEND + 2, F, A
	RLCF FREQUENCY_DIVIDEND + 3, F, A
	; adjust if the dividend becomes too large (prevent overflow)
	MOVLW 0x7F
	BTFSC FREQUENCY_DIVIDEND + 3, 7, A ; if (dividend[3] & 0x80) dividend[3] = 0x7F
	MOVWF FREQUENCY_DIVIDEND + 3, A
	; if nonzero hasn't been found yet, always proceed to next
	MOVF FREQNEUCY_NONZERO_FOUND, W, A
	BZ FREQUENCY_DIVISION_LOOP
	; shift the delta, proceed to next if it didn't reach to zero
	BCF STATUS, C, A
	RRCF FREQUENCY_DELTA + 2, F, A
	RRCF FREQUENCY_DELTA + 1, F, A
	RRCF FREQUENCY_DELTA, F, A
	BNC FREQUENCY_DIVISION_LOOP
	; configure NCO
	...
	TSTFSZ FREQUENCY_POSTSCALE_COUNT, A
	BRA NCO_CONFIGURE_NOT_ONE
	; use one NCO (Fosc -> NCO1 -> CLK)
	MOVFF FREQUENCY_VALUE, NCO1INC
	MOVFF FREQUENCY_VALUE + 1, NCO1INC + 1
	MOVFF FREQUENCY_VALUE + 2, NCO1INC + 2
	MOVLW B'10000000' ; enable, don't invert output, Fixed Duty Cycle mode
	MOVFF WREG, NCO1CON
	BRA NCO_CONFIGURE_END
NCO_CONFIGURE_NOT_ONE
	DECF FREQUENCY_POSTSCALE_COUNT, W, A
	BNZ NCO_CONFIGURE_THREE
	; use two NCOs (Fosc -> NCO2 -> NCO1 -> CLK)
	MOVFF FREQUENCY_POSTSCALE, NCO1INC
	MOVFF FREQUENCY_POSTSCALE + 1, NCO1INC + 1
	MOVFF FREQUENCY_POSTSCALE + 2, NCO1INC + 2
	MOVLW B'00010001' ; clock source = NCO2_OUT
	MOVFF WREG, NCO1CLK
	MOVLW B'10000000' ; enable, don't invert output, Fixed Duty Cycle mode
	MOVFF WREG, NCO1CON
	MOVFF FREQUENCY_VALUE, NCO2INC
	MOVFF FREQUENCY_VALUE + 1, NCO2INC + 1
	MOVFF FREQUENCY_VALUE + 2, NCO2INC + 2
	MOVLW B'10000001' ; enable, don't invert output, Pulse Frequency mode
	MOVFF WREG, NCO2CON
	BRA NCO_CONFIGURE_END
NCO_CONFIGURE_THREE
	; use three NCOs (Fosc -> NCO3 -> NCO2 -> NCO1 -> CLK)
	MOVFF FREQUENCY_POSTSCALE, NCO1INC
	MOVFF FREQUENCY_POSTSCALE + 1, NCO1INC + 1
	MOVFF FREQUENCY_POSTSCALE + 2, NCO1INC + 2
	MOVLW B'00010001' ; clock source = NCO2_OUT
	...
	MOVFF WREG, NCO2INC
	MOVLW 0
	MOVFF WREG, NCO2INC + 1
	MOVFF WREG, NCO2INC + 2
	MOVLW B'00010010' ; clock source = NCO3_OUT
	MOVFF WREG, NCO2CLK
	MOVLW B'10000001' ; enable, don't invert output, Pulse Frequency mode
	MOVFF WREG, NCO2CON
	MOVFF FREQUENCY_VALUE, NCO3INC
	MOVFF FREQUENCY_VALUE + 1, NCO3INC + 1
	MOVFF FREQUENCY_VALUE + 2, NCO3INC + 2
	MOVLW B'10000001' ; enable, don't invert output, Pulse Frequency mode
	MOVFF WREG, NCO3CON
NCO_CONFIGURE_END
...
START_Z80_WAIT_SEND_END
	...
	RESET
	BRA MANAGE_MODE_MAIN_LOOP ; won't come here, for just in case
...
CONFIG_EDIT_CLOCK_ENABLE
	SET_TBLPTR_PUT_STRING CLOCK_ENABLE_PROMPT
CONFIG_EDIT_CLOCK_ENABLE_INPUT_LOOP
	RCALL GET_CHAR
	ADDLW -'0'
	BZ CONFIG_EDIT_CLOCK_ENABLE_SET_OFF
	ADDLW -1
	BZ CONFIG_EDIT_CLOCK_ENABLE_SET_ON
	BRA CONFIG_EDIT_CLOCK_ENABLE_INPUT_LOOP
CONFIG_EDIT_CLOCK_ENABLE_SET_OFF
	CLRF GPR_CLOCK_ENABLE, A
	BRA CONFIG_EDIT_SAVE
CONFIG_EDIT_CLOCK_ENABLE_SET_ON
	SETF GPR_CLOCK_ENABLE, A
	BRA CONFIG_EDIT_SAVE
	; edit clock frequency configuration
CONFIG_EDIT_CLOCK_FREQUENCY
	; initlaize buffer
	SET_FSR 0, EDIT_FREQUENCY_BUFFER
	MOVLW 8 * 2 - 1
CONFIG_EDIT_CLOCK_INITIALIZE_LOOP
	CLRF POSTINC0, A
	ADDLW -1
	BNZ CONFIG_EDIT_CLOCK_INITIALIZE_LOOP
	CLRF EDIT_FREQUENCY_COUNT, A
	; accept input
	SET_TBLPTR_PUT_STRING CLOCK_FREQUENCY_PROMPT
	SET_FSR 0, EDIT_FREQUENCY_BUFFER + 7
CONFIG_EDIT_CLOCK_FREQUENCY_LOOP
	RCALL GET_CHAR
	; Backspace check
	ADDLW -'\b'
	BZ CONFIG_EDIT_CLOCK_FREQUENCY_BACKSPACE
	; Enter check
	ADDLW -('\n' - '\b')
	BZ CONFIG_EDIT_CLOCK_FREQUENCY_ENTER
	ADDLW -('\r' - '\n')
	BZ CONFIG_EDIT_CLOCK_FREQUENCY_ENTER
	; temporary save what is entered
	ADDLW -('0' - '\r')
	MOVWF EDIT_FREQUENCY_INPUT_WORK, A
	; input length check (proceed if length < 8)
	MOVLW 8
	CPFSLT EDIT_FREQUENCY_COUNT, A
	BRA CONFIG_EDIT_CLOCK_FREQUENCY_LOOP
	; input range check (only accept decimal numbers)
	MOVLW 10
	CPFSLT EDIT_FREQUENCY_INPUT_WORK, A
	BRA CONFIG_EDIT_CLOCK_FREQUENCY_LOOP
	; a number is entered
	; remove single-digit "0" if it has been entered
	DECF EDIT_FREQUENCY_COUNT, W, A
	BNZ CONFIG_EDIT_CLOCK_FREQUENCY_NO_SUPPRESS_ZERO
	MOVF EDIT_FREQUENCY_BUFFER + 7, W, A
	BNZ CONFIG_EDIT_CLOCK_FREQUENCY_NO_SUPPRESS_ZERO
	; actually remove "0"
	CLRF EDIT_FREQUENCY_COUNT, A
	MOVLW '\b'
	RCALL PUT_CHAR
CONFIG_EDIT_CLOCK_FREQUENCY_NO_SUPPRESS_ZERO
	; store entered number
	MOVF EDIT_FREQUENCY_COUNT,W, A
	MOVFF EDIT_FREQUENCY_INPUT_WORK, PLUSW0
	INCF EDIT_FREQUENCY_COUNT, F, A
	MOVF EDIT_FREQUENCY_INPUT_WORK, W, A
	ADDLW '0'
	RCALL PUT_CHAR
	BRA CONFIG_EDIT_CLOCK_FREQUENCY_LOOP
CONFIG_EDIT_CLOCK_FREQUENCY_BACKSPACE
	; Backspace is pressed
	MOVLW 0
	CPFSGT EDIT_FREQUENCY_COUNT, A
	BRA CONFIG_EDIT_CLOCK_FREQUENCY_LOOP
	DECF EDIT_FREQUENCY_COUNT, F, A
	MOVF EDIT_FREQUENCY_COUNT,W, A
	CLRF PLUSW0, A
	MOVLW '\b'
	RCALL PUT_CHAR
	...
	RCALL PUT_CHAR
	BRA CONFIG_EDIT_CLOCK_FREQUENCY_LOOP
CONFIG_EDIT_CLOCK_FREQUENCY_ENTER
	; Enter is pressed
	MOVLW 0
	CPFSGT EDIT_FREQUENCY_COUNT, A
	BRA CONFIG_EDIT_CLOCK_FREQUENCY_LOOP
	...
	MOVLW '\n'
	RCALL PUT_CHAR
	; put entered frequency to GPR
	MOVLW HIGH(EDIT_FREQUENCY_BUFFER)
	MOVWF FSR0 + 1, A
	DECF EDIT_FREQUENCY_COUNT, W, A
	ADDLW LOW(EDIT_FREQUENCY_BUFFER)
	MOVWF FSR0, A
	SET_FSR 1, GPR_CLOCK_FREQUENCY
	MOVLW 4
	MOVWF EDIT_FREQUENCY_COUNT, A
CONFIG_EDIT_CLOCK_FREQUENCY_APPLY_LOOP
	SWAPF POSTINC0, W, A
	IORWF POSTINC0, W, A
	MOVWF POSTINC1, A
	DECF EDIT_FREQUENCY_COUNT, F, A
	BNZ CONFIG_EDIT_CLOCK_FREQUENCY_APPLY_LOOP
...
CONFIG_EDIT_SAVE
	...
	MOVWF NVMADR + 2, B
	MOVLW GPR_CONFIG_END - GPR_CONFIG_BEGIN
	MOVWF CONFIG_SAVE_COUNTER, A
	SETF CONFIG_SAVE_OK, A
	SET_FSR 0, GPR_CONFIG_BEGIN
CONFIG_EDIT_SAVE_LOOP
	; read current EEPROM contents
	CLRF NVMCON1, B ; Read
	BSF NVMCON0, GO, B
	MOVF NVMDATL, W, B
	SUBWF INDF0, W, A
	BZ CONFIG_EDIT_WRITE_END
	; write to EEPROM (only if the contents differ)
	MOVFF INDF0, NVMDATL
	MOVLW B'00000011' ; Write
	MOVWF NVMCON1, B
	RCALL NVM_OPERATION
CONFIG_EDIT_WRITE_END
	...
	MOVF NVMDATL, W, B
	CPFSEQ INDF0, A
	CLRF CONFIG_SAVE_OK, A ; set error flag if the contents differs
	MOVWF POSTINC0, A ; set GPR contents to EEPROM contents
	; proceed to next byte
	DECF CONFIG_SAVE_COUNTER, F, A
	BNZ CONFIG_EDIT_SAVE_LOOP
	; return to configuration menu (to show new configuration)
	MOVLW '\r'
	RCALL PUT_CHAR
	MOVLW '\n'
	RCALL PUT_CHAR
	TSTFSZ CONFIG_SAVE_OK, A
	BRA CONFIG_MENU
	SET_TBLPTR_PUT_STRING WRITE_ERROR_MESSAGE
	BRA CONFIG_MENU
...
CLOCK_ENABLE_PROMPT
	DA "\r\nnew clock output mode? (0: OFF / 1: ON)\r\n\0"

CLOCK_FREQUENCY_PROMPT
	DA "\r\nnew clock frequency [Hz] : \0"

WRITE_ERROR_MESSAGE
	DA "failed to save configuration!\r\n\r\n\0"
